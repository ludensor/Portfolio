...
#include "EQSNavSampleEnvQueryGenerator_TriangleTessellation.h"
...
UEQSNavSampleEnvQueryGenerator_TriangleTessellation::UEQSNavSampleEnvQueryGenerator_TriangleTessellation(const FObjectInitializer& ObjectInitializer)
...
float UEQSNavSampleEnvQueryGenerator_TriangleTessellation::GetArcBisectorAngle(FEnvQueryInstance& QueryInstance) const
{
...
void UEQSNavSampleEnvQueryGenerator_TriangleTessellation::GetEdgeVectors(float BisectAngleDegrees, float Edge, TArray<FVector>& OutVectors) const
{
	...
	for (int32 SectionIndex = 0; SectionIndex < 3; ++SectionIndex, SectionAngle += AngleDelta)
	{
...
int32 UEQSNavSampleEnvQueryGenerator_TriangleTessellation::GetItemCount(float Radius, const FVector& FirstEdge, const FVector& SecondEdge, int32 StepsCount) const
{
...
bool UEQSNavSampleEnvQueryGenerator_TriangleTessellation::IsAngleAllowed(const FVector& Point, float BisectAngleDegrees, float AngleRangeDegrees, bool bConstrainAngle) const
{
	if (bConstrainAngle)
	{
...
void UEQSNavSampleEnvQueryGenerator_TriangleTessellation::GenerateItems(FEnvQueryInstance& QueryInstance) const
{
...
FText UEQSNavSampleEnvQueryGenerator_TriangleTessellation::GetDescriptionTitle() const
{
...
FText UEQSNavSampleEnvQueryGenerator_TriangleTessellation::GetDescriptionDetails() const
{
...
#if WITH_EDITOR
void UEQSNavSampleEnvQueryGenerator_TriangleTessellation::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	if (PropertyChangedEvent.Property)
	{
		const FName PropName = PropertyChangedEvent.MemberProperty->GetFName();
		if (PropName == GET_MEMBER_NAME_CHECKED(UEQSNavSampleEnvQueryGenerator_TriangleTessellation, ArcAngle))
		{
		...
		else if (PropName == GET_MEMBER_NAME_CHECKED(UEQSNavSampleEnvQueryGenerator_TriangleTessellation, CircleRadius))
		{
			CircleRadius.DefaultValue = FMath::Max(CircleRadius.DefaultValue, FMath::Max(MIN_CIRCLE_RADIUS, TriangleEdge.DefaultValue));
		}
		else if (PropName == GET_MEMBER_NAME_CHECKED(UEQSNavSampleEnvQueryGenerator_TriangleTessellation, TriangleEdge))
		{
